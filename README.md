...
4. The office inbox lives at `/office/messages`.
5. The contact form submits to `/api/contact` and the inbox reads from `/api/messages`.
...
4. Set `POSTGRES_SSL=false` only if you are connecting to a local PostgreSQL server that does not use SSL.
